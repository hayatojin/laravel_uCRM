...
const toggleStatus = () => { isShow.value = !isShow.value}  // モーダルを閉じる・開くの処理

// async await = await側の処理が終わるまで、他の処理を止める
// AjaxでDBへ接続する処理のため、DB接続の場合は、try/catchでエラー時の対処もしておく
const searchCustomers = async () => {
    ...
    toggleStatus()  // モーダルを閉じる・開くの処理
    } catch (e){
        console.log(e)
    }
}

// Emitで子から親へ情報アップ（vueの検証ツール使えば、micromodalが子どもにあたると分かる）
const emit = defineEmits(['update:customerId']) 

const setCustomer = e => {
    search.value = e.kana
    emit('update:customerId', e.id) // emitは、第一引数はイベント名を設定する　子のイベントを親に送るイメージ
    toggleStatus()
}
